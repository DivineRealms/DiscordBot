...
module.exports.automod = async(client, message) => {
  ...
  if (message.mentions.users.first() && client.afk.has(message.mentions.users.first().id)) {
    ...
    const embed5 = new client.embedBuilder(client, message,
  ...
  if (settings.Max_User_Pings && message.mentions.users.size > settings.Max_User_Pings && !message.member.permissions.has("MANAGE_GUILD")) {
    ...
    return message.channel.send({ embeds: [new client.embedBuilder(client, message, "Error", `Sorry but you can't ping more than ${settings.Max_User_Pings} users at once!`, "RED")]})
    ...
    await message.delete()
    return message.channel.send({ embeds: [new client.embedBuilder(client, message, "Error", `Sorry but you can't ping more than ${settings.Max_Role_Pings} roles at once!`, "RED")]})
    ...
    await message.delete()
    return message.channel.send({ embeds: [new client.embedBuilder(client, message, "Error", `Sorry but you can't ping more than ${settings.Max_Channel_Pings} channels at once!`, "RED")]})
  } else if (settings.Max_Total_Pings && totalPings > settings.Max_Total_Pings && !message.member.permissions.has("MANAGE_GUILD")) {
    await message.delete()
    return message.channel.send({ embeds: [new client.embedBuilder(client, message, "Error", `Sorry but you can't ping more than ${settings.Max_Total_Pings} times in 1 message!`, "RED")]})
  }
  ...
  if (message.channel.id === client.conf.counting.Counting_Channel) {
    ...
    if (message.content != current) {
      ...
      return message.channel.send({ embeds: [new client.embedBuilder(client, message, "Error",
    ...
    if (client.conf.counting.One_At_A_Time && last === message.author.id && current !== 1) {
      message.delete()
      if (current !== 1 && client.conf.counting.Restart_On_Incorrect_Number) client.settings.set(message.guild.id, 1, 'counting.current')
      return message.channel.send({ embeds: [new client.embedBuilder(client, message, "Warning", client.conf.counting.One_At_A_Time_Message.replace('{username}', message.author.username) + '\n' +
  ...
  if (msgs >= 8 && settings.Enable_Spam) {
    ...
    message.member.roles.add(client.conf.moderation.Mute_Role).then(() => {
      message.channel.send({ embeds: [new client.embedBuilder(client, message, "Warning", client.resolveMember(settings.Spam_Message, message.author))]})
  ...
  if (settings.Caps_Limit.match(/\d/g) && percent > settings.Caps_Limit.match(/\d+/g)[0] && message.content.length >= settings.Caps_Minimum_Characters) {
    ...
    message.delete().then(() => {
      message.channel.send({ embeds: [new client.embedBuilder(client, message, "Warning", settings.Max_Caps_Message.replace('{member}', message.author.toString()), "ORANGE")]})
    }).catch(() => {})
  }
  ...
  if (attch && !message.member.permissions.has("MANAGE_GUILD") && !settings.Bypass_Attachments_Roles.some(r => message.member.roles.cache.has(r))) {
    message.delete().then(() => {
      message.channel.send({ embeds: [new client.embedBuilder(client, message, "Warning", settings.Banned_Attachments_Message.replace('{member}', message.author.toString()).replace('{attachment}', attch), "ORANGE")]})
    }).catch(() => {})
  }
  ...
  if (!message.member.permissions.has("MANAGE_GUILD") && settings.Banned_Words.some(a => message.content.toLowerCase().includes(a))) {
    ...
    message.delete().then(() => {
      message.channel.send({ embeds: [new client.embedBuilder(client, message, "Warning", settings.Banned_Words_Message.replace('{member}', message.author.toString()), "ORANGE")]})
    }).catch(() => {})
  }
  ...
  if (!message.member.permissions.has("MANAGE_GUILD") && settings.Banned_Emojis.some(a => message.content.toLowerCase().includes(a))) {
    ...
    message.delete().then(() => {
      message.channel.send({ embeds: [new client.embedBuilder(client, message, "Warning", settings.Banned_Emojis_Message.replace('{member}', message.author.toString()), "ORANGE")]})
    }).catch(() => {})
  }
  ...
  if (!message.member.permissions.has("MANAGE_GUILD") && settings.Banned_Links.some((a, i) => message.content.toLowerCase().includes(a) && !message.content.includes(settings.Allowed_Domains[i]))) {
    ...
    message.delete().then(() => {
      message.channel.send({ embeds: [new client.embedBuilder(client, message, "Warning", settings.Banned_Links_Message.replace('{member}', message.author.toString()), "ORANGE")]})
    }).catch(() => {})
  }
}