...
module.exports.run = async(client, message, args) => {
    ...
    if (!args.join(' ').match(/<a?:(\w{1,32}):(\d{17,19})>/g)) return message.channel.send(new client.embed().setDescription(`Please write some custom emojis for me to steal!`).setFooter(message.author.username, message.author.displayAvatarURL({ dynamic: true, size: 1024 })))

    const emojis = [...args.join(' ').matchAll(/<a?:(\w{1,32}):(\d{17,19})>/g)].map(s => [s[1], s[2]])
    if (emojis.length > 5) return message.channel.send(new client.embed().setDescription(`You cant create more than 5 emojis at once!`).setFooter(message.author.username, message.author.displayAvatarURL({ dynamic: true, size: 1024 })))
    const created = []
