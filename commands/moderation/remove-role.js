...
module.exports.run = async(client, message, args) => {
    ...
    if (!member.roles.cache.has(role.id)) return message.channel.send(new client.embed().setDescription(`That member doesnt have the role ${role}!`).setFooter(message.author.username, message.author.displayAvatarURL({ dynamic: true, size: 1024 }))).then(m => m.delete({ timeout: 9000 }))
    ...
    return member.roles.remove(role).then(() =>
        message.channel.send(embed)).catch(() => message.channel.send(new client.embed().setDescription('I cant remove that role from that member!').setFooter(message.author.username, message.author.displayAvatarURL({ dynamic: true, size: 1024 }))).then(m => m.delete({ timeout: 9000 })))
}