...
module.exports.run = async(client, message, args) => {
    ...
    if (!message.member.permissions.has("BAN_MEMBERS")) return message.channel.send(embed.setDescription('You can\'t use that you dumb fuck!'));
    if (!message.guild.me.permissions.has("BAN_MEMBERS")) return message.channel.send(embed.setDescription(`Sorry, I am missing my required permissions perhaps try moving my role up!`));

    const member = message.mentions.users.first() || await client.users.fetch(args[0]).catch(() => {});

    if (!args[0]) return message.channel.send({ embeds: [embed.setDescription('Please specify a user to ban!')]});
    if (!member) return message.channel.send({ embeds: [embed.setDescription('Sorry, I can\'t seem to find this user!')]});
    if (member.id === message.author.id) return message.channel.send({ embeds: [embed.setDescription('Why are you trying to ban yourself?')]})
    if (message.guild.member(member) && !message.guild.member(member).bannable) return message.channel.send({ embeds: [new client.embed().setDescription('I cant ban that user, my role must be higher in order to do that')]})
