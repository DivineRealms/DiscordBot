...
module.exports.run = async(client, message, args) => {
    ...
    member.send(embed.setTitle(`You were warned in ${message.guild.name}`)).catch(() =>
        message.channel.send(new client.embed().setDescription(`Failed to send a dm to ${member}, their dms are locked. FUCK.. I WANTED TO PISS THEM OFF!`).setFooter(message.author.username, message.author.displayAvatarURL({ dynamic: true, size: 1024 }))))
