...
module.exports.run = async(client, message, args) => {
    ...
    if (!message.member.permissions.has("MUTE_MEMBERS"))
        return message.channel.send({ embeds: [new client.embed().setDescription(`You are missing permission \`MUTE_MEMBERS\``).setFooter(message.author.username, message.author.displayAvatarURL({ dynamic: true, size: 1024 }))]});
    ...
    let dm = await member.send({ embeds: [embed2.setTitle(`You have been unmuted`)]}).catch(() => {})
    if (!dm) message.channel.send({ embeds: [new client.embed().setDescription(`Failed to send dm to ${member}, they have dms off!`)]})
}