...
module.exports.run = async(client, message, args) => {
    ...
    if (!args[0] || (isNaN(args[0]) && args[0] !== 'all')) return message.channel.send({ embeds: [client.embedBuilder(client, message, "You must provide amount to deposit.", "", "error")] });

    if (args[0] === 'all') {
      if (!bank || bank == 0) return message.channel.send({ embeds: [client.embedBuilder(client, message, "You don't have money to withdraw.", "", "error")] });
    ...
    if (args[0] > bank) return message.channel.send({ embeds: [client.embedBuilder(client, message, "You cannot withdraw that much.", "", "error")] });
    if (args[0] < 1) return message.channel.send({ embeds: [client.embedBuilder(client, message, "You cannot withdraw less than $1.", "", "error")] });
