...
module.exports.run = async(client, message, args) => {
  ...
  if (!args[0] || (isNaN(args[0]) && args[0] !== 'all')) return message.channel.send({ embeds: [client.embedBuilder(client, message, "Error", "You need to enter how much to bet.", "error")] });
  if (!bal || bal == 0) return message.channel.send({ embeds: [client.embedBuilder(client, message, "Error", "You don't have money to bet.", "error")] });
  ...
  if (args[0] < 200) return message.channel.send({ embeds: [client.embedBuilder(client, message, "Error", "You cannot bet less than $200.", "error")] });
