...
module.exports.run = async(client, message, args) => {
    ...
    if (!data.inventory.items.some(s => s.name.includes('Ring'))) return message.channel.send(new client.embed().setDescription(`You need to buy a ring to propose! Buy one from the \`${message.px}shop\`!`).setFooter(message.author.username, message.author.displayAvatarURL({ dynamic: true, size: 1024 })))

    if (!proposed) return message.channel.send(new client.embed().setDescription('Please mention a user to propose to!').setFooter(message.author.username, message.author.displayAvatarURL({ dynamic: true, size: 1024 })))
    ...
    message.channel.send(embed).then(() => {
        message.channel.awaitMessages(r => r.author.id == proposed.id && r.content.toLowerCase().includes('yes'), { max: 1, time: 8000 })
            .then(collected => {
                if (!collected.first()) return message.channel.send(new client.embed().setDescription(`Looks like ${proposed.username} didn\'t feel the same way :c`).setFooter(message.author.username, message.author.displayAvatarURL({ dynamic: true, size: 1024 })))

                message.channel.send(new client.embed().setDescription(`${collected.first().author} said yes! How heart warming :sparkling_heart:! You are now engaged!`)).setFooter(message.author.username, message.author.displayAvatarURL({ dynamic: true, size: 1024 }))
