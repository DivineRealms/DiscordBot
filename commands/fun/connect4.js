...
module.exports.run = async(client, message, args, cmd) => {
  ...
  if (!message.mentions.users.first() || message.mentions.users.first().id == message.author.id || message.mentions.users.first().bot) return message.channel.send({ embeds: [client.embedBuilder(client, message, "You need to mention valid user you want to play against.", "", "error")] });
  if (games.get(message.guild.id + message.mentions.users.first().id)) return message.channel.send({ embeds: [client.embedBuilder(client, message, "That User is already playing.", "", "error")] });
  if (requests.get(message.guild.id + message.mentions.users.first().id)) return message.channel.send({ embeds: [client.embedBuilder(client, message, "That User has already sent request to someone else.", "", "error")] });
  ...
  message.channel.awaitMessages(filter1, { max: 1, time: 8000 })
    .then(collected => {
      if (!collected.first()) return message.channel.send({ embeds: [client.embedBuilder(client, message, "They didn't confirm in time.", "", "error")] });
      ...
      const embed = client.embedBuilder(client, message,
        `${message.author.username} vs. ${message.mentions.users.first().username}`, board(game).join('\n').replace(/,/g, ''))
      ...
      message.channel.send({ embeds: [embed] }).then(emb => {
        ...
        collector.on('end', () => {
          ...
          else message.channel.send({ embeds: [client.embedBuilder(client, message, "Time Limit has reached and there's no winners.", "", "error")] });
