...
module.exports.run = async(client, message, args) => {
    ...
    if (!channel) return message.channel.send(new client.embed().setDescription('A report channel hasnt been setup for this server!').setFooter(message.author.username, message.author.displayAvatarURL({ dynamic: true, size: 1024 })))
    if (!args[0]) return message.channel.send(new client.embed().setDescription(`Please provide me a report!`).setFooter(message.author.username, message.author.displayAvatarURL({ dynamic: true, size: 1024 })));
