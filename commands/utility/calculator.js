...
module.exports.run = async(client, message, args) => {
    ...
    try {
        message.channel.send({ embeds: [new client.embed().setTitle('Here dumbo cant do basic math huh?')
        ...
        message.channel.send({ embeds: [new client.embed().setDescription('Please provide me an equation.').setFooter(message.author.username, message.author.displayAvatarURL({ dynamic: true, size: 1024 }))]})
    }
}